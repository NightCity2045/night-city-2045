access-level-ncpd-patrol = Патруль
access-level-ncpd-armory = Оружейная
access-level-ncpd-detective = Детектив
access-level-ncpd-command = Командование NCPD

access-level-maxtac-general = MaxTac
access-level-maxtac-armory = Оружейная MaxTac
access-level-maxtac-command = Командир MaxTac

access-level-trauma-medical = Медблок
access-level-trauma-morgue = Морг
access-level-trauma-hangar = Ангар
access-level-trauma-command = Главврач TT

access-level-nc-general = Житель Найт-Сити
access-level-edgerunner = Эджраннер
access-level-fixer = Фиксер
access-level-ripper = Рипер
access-level-cafe = Кафе
access-level-restaurant = Ресторан
access-level-zhirafa-janitor = Уборщик Жирафы

access-level-militech-command = Командование Militech
access-level-militech-operative = Оперативники Militech
access-level-militech-armory = Оружейная Militech
access-level-militech-tech = Технический отдел Militech
access-level-militech-medical = Медицинский отдел Militech
access-level-militech-staff = Персонал Militech
access-level-militech-general = Militech

